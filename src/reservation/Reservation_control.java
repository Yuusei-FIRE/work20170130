...
public String getExplanation(String facility/*Reservation_view frame*/ ){
	//String facility ="";
	String res="";
	...
	if ( flagLogin){ // ログインしていた場合
		if( facility=="小ホール "){
		res ="小ホール・・・\n" 
		    + "収容人数は20名程度、片面の壁は鏡張りです。\n\n";}
		
		else if(facility ==" 大会議室1") {
		res = "大会議室１・・・\n" 
		    + "収容人数は100名程度、円テーブルが6つあります。\n\n";}
		
		else if(facility == " 大会議室2") {
		res = "大会議室2・・・\n"
		    +"収容人数は500名程度、大きなステージがあります席の背もたれに番号がついており非常口は2つあります\n\n";}
		
		else if(facility ==" 小会議室1"){
		res = "小会議室１・・・\n"
		    + "収容人数は10名。机が一つあり、ホワイトボードが壁に添えつけてあります。\n\n";}
		
		else if(facility ==" 小会議室2"){
		res = "小会議室2・・・\n"
		    + "収容人数は10名、小会議室2と作りは同じです。\n\n";}
		
		else if(facility ==" 小会議室3"){
		res = "小会議室3・・・\n"
		    + "収容人数は10名、プロジェクターを使うことができます。\n\n";}
		
		else if (facility ==" 小会議室4"){
        res = "小会議室4・・・\n"
		    + "収容人数は15名、ホワイトボードとプロジェクターがあり、円形の机があります。\nwi-fi環境がありインターネットが無料で使えます。\n\n";}
		
		else if (facility ==" 小会議室5"){
	    res = "小会議室5・・・\n"
		    + "収容人数は30名、小会議室の中では一番大きいです。それぞれのテーブルにモニターが備え付けてあります。\n\n";}
	
		else if(facility == " 小会議室6"){
		res =  "小会議室6・・・\n"
		    + "収容人数は30名、小会議室5と同じです。\n\n"
...
return res;
    ...
    if ( flagLogin){ // ログインしていた場合
ReservationDialoging rd = new ReservationDialoging(frame);
